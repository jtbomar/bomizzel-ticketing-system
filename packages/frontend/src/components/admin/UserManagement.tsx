...
const UserManagement: React.FC = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      try {
        ...
        if (filters.isActive) {
          filteredUsers = filteredUsers.filter(user => 
            user.isActive === (filters.isActive === 'true')
          );
        }
  ...
  const handleEditUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Show success message
      alert(`Agent ${editingUser.firstName} ${editingUser.lastName} has been updated successfully!`);
            ...
            {users.map((user) => (
              <tr key={user.id} className="hover:bg-white/10 transition-all duration-200">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  >
                    <option value="customer" style={{ backgroundColor: '#1e293b', color: 'white' }}>Customer</option>
                    <option value="employee" style={{ backgroundColor: '#1e293b', color: 'white' }}>Agent</option>
          ...
          <div className="bg-white/10 backdrop-blur-lg rounded-xl border border-white/20 p-6 w-full max-w-md mx-4">
            ...
            <form onSubmit={handleAddUser} className="space-y-4">
              ...
              <div>
                ...
                <select
                  value={newUser.role}
                  onChange={(e) => setNewUser(prev => ({ ...prev, role: e.target.value as 'customer' | 'employee' | 'admin' }))}
              ...
              <div>
                ...
                <select
                  ...
                  style={{ backgroundColor: 'rgba(255,255,255,0.1)', color: 'white' }}
                >
                  <option value="" style={{ backgroundColor: '#1e293b', color: 'white' }}>No Team (Admin Only)</option>
                  {teams.map(team => (
                    <option key={team.id} value={team.id} style={{ backgroundColor: '#1e293b', color: 'white' }}>{team.name}</option>
                  ))}
                </select>
              </div>
            ...
            <form onSubmit={handleEditUser} className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setEditingUser(prev => prev ? { ...prev, role: e.target.value as 'customer' | 'employee' | 'admin' } : null);
                  }}
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setEditingUser(prev => prev ? { ...prev, teamId: e.target.value } as any : null)}
                  className="block w-full rounded-md bg-white/10 border border-white/20 text-white shadow-sm focus:border-blue-400 focus:ring-blue-400 sm:text-sm px-3 py-2"
                  style={{ backgroundColor: 'rgba(255,255,255,0.1)', color: 'white' }}
                >
                  <option value="" style={{ backgroundColor: '#1e293b', color: 'white' }}>No Team (Admin Only)</option>
              ...
              <div>
                ...
                <select
                  ...
                  style={{ backgroundColor: 'rgba(255,255,255,0.1)', color: 'white' }}
                >
                  <option value="true" style={{ backgroundColor: '#1e293b', color: 'white' }}>Active</option>
                  <option value="false" style={{ backgroundColor: '#1e293b', color: 'white' }}>Inactive</option>
                </select>
              </div>
