...
const BulkOperations: React.FC<BulkOperationsProps> = ({
  ...
  const [operation, setOperation] = useState<'assign' | 'status' | 'priority' | 'move' | 'delete' | null>(null);
  ...
  const handleOperation = async () => {
    ...
    try {
      ...
      switch (operation) {
        ...
        case 'delete':
          if (!confirm(`Are you sure you want to delete ${ticketIds.length} tickets? This action cannot be undone.`)) {
            return;
          }
              ...
              <div className="flex">
                ...
                <div>
                  ...
                  <p className="text-sm text-red-700 mt-1">
                    You are about to delete {ticketIds.length} tickets. This will permanently remove them from the system.
                  </p>
                </div>
              </div>
            </div>
        ...
        {!result ? (
              ...
              <div className="space-y-4">
                <div className="flex items-center space-x-2">
                  ...
                  <h4 className="text-sm font-medium text-gray-700">
                    {operation.charAt(0).toUpperCase() + operation.slice(1)} {ticketIds.length} tickets
                  </h4>
                </div>
                ...
                <div className="flex justify-end space-x-3 pt-4">
                  ...
                  >
                    {loading ? 'Processing...' : `${operation.charAt(0).toUpperCase() + operation.slice(1)} Tickets`}
                  </button>
                </div>
              </div>
            )}
          ...
          <div className="space-y-4">
            <div className={`p-4 rounded-md ${result.success ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
              <div className="flex">
                <svg className={`w-5 h-5 mr-2 ${result.success ? 'text-green-400' : 'text-red-400'}`} fill="currentColor" viewBox="0 0 20 20">
                  {result.success ? (
