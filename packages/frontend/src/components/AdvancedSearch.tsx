    ...
    if (needsList) {
      return (
        <textarea
          ...
          onChange={(e) => updateFilter(index, { values: e.target.value.split('\n').filter(v => v.trim()) })}
                ...
                {filters.map((filter, index) => (
                  <div key={index} className="grid grid-cols-12 gap-3 items-start">
                    <div className="col-span-3">
                      <select
                        value={filter.field}
                        onChange={(e) => updateFilter(index, { field: e.target.value, value: '', values: [] })}
                        className="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                      >
                        <option value="">Select field...</option>
                        {searchableFields.filter(f => f.searchable).map(field => (
                    ...
                    <div className="col-span-2">
                      <select
                        ...
                        onChange={(e) => updateFilter(index, { operator: e.target.value, value: '', values: [] })}
                        className="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                        disabled={!filter.field}
                      >
                        {filter.field && searchableFields.find(f => f.name === filter.field) ? 
                          getOperatorsForField(searchableFields.find(f => f.name === filter.field)!).map(op => (
