...
const SubscriptionManagement: React.FC<SubscriptionManagementProps> = ({ className = '' }) => {
  ...
  const renderUsageMeter = (label: string, current: number, limit: number, percentage: number) => {
                ...
                .map((availablePlan) => (
                  <div key={availablePlan.id} className="border border-gray-200 rounded-lg p-4">
                    ...
                    <div className="text-xs text-gray-500 mb-3">
                      <p>
                        Active:{' '}
                        {availablePlan.limits.activeTickets === -1
                          ? 'Unlimited'
                          : availablePlan.limits.activeTickets}
                      </p>
                      <p>
                        Completed:{' '}
                        {availablePlan.limits.completedTickets === -1
                          ? 'Unlimited'
                          : availablePlan.limits.completedTickets}
                      </p>
                    </div>
                    <button
                  ...
                  {subscription.paymentMethodId && (
                    <div>
                      ...
                      <div className="flex items-center justify-between">
                        <p className="text-sm text-gray-900">
                          •••• •••• •••• {subscription.paymentMethodId.slice(-4)}
                        </p>
          ...
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            ...
            <div className="mb-6">
              ...
              <div className="bg-blue-50 rounded-lg p-4">
                ...
                <p className="text-sm text-gray-600 mb-2">
                  {selectedUpgradePlan.name} - {formatCurrency(selectedUpgradePlan.price)} per{' '}
                  {selectedUpgradePlan.billingInterval}
                </p>
                <div className="text-xs text-gray-500">
                  <p>
                    Active Tickets:{' '}
                    {selectedUpgradePlan.limits.activeTickets === -1
                      ? 'Unlimited'
                      : selectedUpgradePlan.limits.activeTickets}
                  </p>
                  <p>
                    Completed Tickets:{' '}
                    {selectedUpgradePlan.limits.completedTickets === -1
                      ? 'Unlimited'
                      : selectedUpgradePlan.limits.completedTickets}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-yellow-50 border border-yellow-200 text-yellow-700 px-4 py-3 rounded mb-6">
              <p className="text-sm">
                <strong>Note:</strong> Your plan will be upgraded immediately and you'll be charged
                the prorated amount for the remainder of your current billing period.
              </p>
            </div>
