...
}), ...enforceAndTrackTicketCreation, async (req, res, next) => {
  try {
...
}), addUsageWarnings, async (req, res, next) => {
  try {
    const userId = req.user!.id;
...
}), ...enforceAndTrackTicketStatusChange, async (req, res, next) => {
  try {
    const ticketId = req.params.id;
 ...
 */
router.post('/:id/assign', validateRequest({
  params: {
    id: { type: 'string', required: true, format: 'uuid' },
  },
  body: {
 ...
 */
router.post('/:id/unassign', validateRequest({
  params: {
    id: { type: 'string', required: true, format: 'uuid' },
  },
}), addUsageWarnings, async (req, res, next) => {
...
router.put('/:id/status', validateRequest({
  ...
  try {
    ...
    const middleware = isCompletion ? enforceAndTrackTicketCompletion : enforceAndTrackTicketStatusChange;
 ...
 */
router.put('/:id/priority', validateRequest({
  params: {
    id: { type: 'string', required: true, format: 'uuid' },
  },
  body: {
 ...
 */
router.post('/:id/attachments', validateRequest({
  params: {
    id: { type: 'string', required: true, format: 'uuid' },
  },
}), uploadSingle, handleMulterError, addUsageWarnings, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const ticketId = req.params.id;
    const userId = req.user!.id;
 ...
 */
router.get('/:id/attachments', validateRequest({
  params: {
    id: { type: 'string', required: true, format: 'uuid' },
  },
}), addUsageWarnings, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const ticketId = req.params.id;
    const userId = req.user!.id;
 ...
 */
router.get('/:id/history', validateRequest({
  params: {
    id: { type: 'string', required: true, format: 'uuid' },
  },
}), addUsageWarnings, async (req: Request, res: Response, next: NextFunction) => {
