 ...
 */
router.get('/users', authenticate, requireAdmin, validateRequest({
  query: {
    page: { type: 'number', required: false, min: 1 },
    limit: { type: 'number', required: false, min: 1, max: 100 },
    role: { type: 'string', required: false, enum: ['customer', 'employee', 'team_lead', 'admin'] },
 ...
 */
router.get('/users/:userId', authenticate, requireAdmin, validateRequest({
  params: {
 ...
 */
router.put('/users/:userId/role', authenticate, requireAdmin, validateRequest({
  params: {
    userId: { type: 'string', required: true, format: 'uuid' },
  },
  body: {
    role: { type: 'string', required: true, enum: ['customer', 'employee', 'team_lead', 'admin'] },
  },
}), async (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
 ...
 */
router.put('/users/:userId/status', authenticate, requireAdmin, validateRequest({
  params: {
    userId: { type: 'string', required: true, format: 'uuid' },
  },
  body: {
 ...
 */
router.post('/users', authenticate, requireAdmin, validateRequest({
  body: {
 ...
 */
router.put('/users/:userId', authenticate, requireAdmin, validateRequest({
  params: {
    userId: { type: 'string', required: true, format: 'uuid' },
  },
  body: {
